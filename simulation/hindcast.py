...
Usage:
    python simulation/hindcast.py                    # run all hindcasts
    python simulation/hindcast.py --system solar     # single subsystem
    python simulation/hindcast.py --plot             # show plots
...
def run_hindcast(params: Optional[SystemParameters] = None,
    ...
    for sys_name in systems:
        obs_data = ALL_OBSERVED[sys_name]
        obs_years = sorted([yr for yr in obs_data if start_year <= yr <= end_year])
        if len(obs_years) < 2:
            continue
        ...
        pred_obs = energy_to_observations(sys_name, t_obs, E_interp)
