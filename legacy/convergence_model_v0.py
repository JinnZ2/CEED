...
if __name__ == "__main__":
    ...
    print("Running CEED Convergence Model...")
    print("=" * 50)

    t, solution = predictor.predict_convergence(years=3)
    total_energy, phases = predictor.classify_phases(solution)

    print(f"  Start: {total_energy[0]:.1f} units")
    print(f"  End: {total_energy[-1]:.1f} units")
    print(f"  Peak phase: {max(phases)}")
